...
def buscar(rotulos, elemento): #lo que hace esta funcion es buscar un elemento y reducir la altura para que sea mas facil de buscar
    ...
    while j!=rotulo:
        [rotulos[j], j]= [rotulo, rotulos[j]] # aqui entonces rotulo es 0 , entonces el elemenoto donde esta el rotulo lo iguala al rotulo rpincipal, y i lo iguala
                                                #al elemenoto en la psocion i.

    return [rotulo,rotulos] #retornamos el rotulo princiapl, y el roden de los rotulos en "compresion"


def fusionar(rotulos,rangos,rotulo1,rotulo2): #Se usa para no tener separadas dos monituclos, por decirlo asi, es decir: [0,0,0,3,3] ->(fusionar)-> [0,0,0,0,3]
    a = min(rotulo1,rotulo2) #sacamos al minimo
    b = max(rotulo1,rotulo2) #y al maximo de los rotulos
    if rangos[a] == rangos[b]: #si ambos rangos son iguales, lo que hacemos es fusionar, y aumentar de rango al a que pas aa ser el principal
        rotulos[b] =a
        rangos[a]+=1
    elif rangos[a]>rangos[b]: #si el rango a es mayor al de b entonces el rtoulo de b es igual al de a, uniendolo sin ponerle rango porque se conserva
        rotulos[b] = a
...
def construir(conjuntos):
    totalE = [item for sublist in conjuntos for item in sublist] #obtenemos el total de elemenos quitando los agrupados
    #lenE = len(totalE)
    rotulos = []
...
def planificacion(tareas):
    ...
    plazos = [0]
    numTareas = len(tareas)
    for tarea in tareas:
        plazos.append(tarea.getPlazo())
    maximoPlazo = min(numTareas,max(plazos)) # Obtenemos cual es el maximo plazo que pueda ser añadido que no alcance el maximo de celdas

    resultado = [0]*maximoPlazo # Creamos un array de 0 [0,0,0,0,...] -> para añadir en como seran las tareas
    menores = [range(maximoPlazo+1)] # creamos un array ordenado para luego ver como ordenar en rotulos [0,1,2,3,4,...]

    conjuntosIni = [[k] for k in range(maximoPlazo)] #los conjuntos iniciales serian los primeros rotulos individuales de rango 0

    [rotulos,rangos, elementos] = construir(conjuntosIni) #construiimos los conjuntos iniciales y los vamos a guardar en las variables del retorno
    #haremos un for en el tamaño de las tareas:
    for p in range(numTareas):
        ...
        menor = menores[rotulo] #escoje como menor al menor emdiante la psocion del rotulo escogido
        if menor>0:
...
print(planificacion(tareasPrediseñadas()))
